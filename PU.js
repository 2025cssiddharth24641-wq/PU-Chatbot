...
function sendMessage() {
  ...
  setTimeout(() => {
    ...
    if (result.confidence === 'high' && result.label) {
      finalReply = `<span style="display:inline-block;background:rgba(0,229,255,0.1);border:1px solid rgba(0,229,255,0.3);border-radius:10px;padding:5px 12px;margin-bottom:10px;font-size:12px;color:#00E5FF;">✨ Showing results for <strong>${result.label}</strong></span><br>` + finalReply;
    } else if (result.confidence === 'low' && result.label) {
      finalReply = `<span style="display:inline-block;background:rgba(176,38,255,0.1);border:1px solid rgba(176,38,255,0.3);border-radius:10px;padding:5px 12px;margin-bottom:10px;font-size:12px;color:#B026FF;">💡 Did you mean <strong>${result.label}</strong>?</span><br>` + finalReply;
    }
...
document.body.addEventListener('click', (e) => {
  ...
  e.preventDefault();
  const sidebarItem = interactiveElement.closest('.sidebar .nav-item');
  if (sidebarItem) {
